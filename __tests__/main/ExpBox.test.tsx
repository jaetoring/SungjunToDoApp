...
describe("ExpBox", () => {
  ...
  it("경험치 바가 올바르게 렌더링 된다.", () => {
    const { getByTestId } = render(<ExpBox currentExp={30} level={21} />);
  ...
  it("현재 경험치가 0일 때, 모든 블록이 비어있어야 한다.", () => {
    const { getAllByTestId } = render(<ExpBox currentExp={0} level={21} />);
  ...
  it("경험치가 100을 넘었을 때, 레벨업하고 나머지가 처리되야 한다.", () => {
    const { getByTestId, getAllByTestId } = render(
      <ExpBox currentExp={105} level={21} />
    );
  ...
  // 경험치 값이 올바르게 출력되는지
  it("경험치 값이 올바르게 출력된다.", () => {
    const { getByText } = render(<ExpBox currentExp={30} level={21} />);
